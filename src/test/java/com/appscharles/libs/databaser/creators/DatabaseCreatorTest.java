...
import com.appscharles.libs.databaser.builders.H2ServerBuilder;
...
public class DatabaseCreatorTest extends TestCase {
    ...
    public void shouldTestCreateDatabaseIfNotExist() throws IOException, DatabaserException, SQLException {
        ...
        IServer server = H2ServerBuilder.create(3456,  dbDir).build();
        server.start();
        IDatabaseCreator creator = new DatabaseCreator("tcp://localhost:3456/myDB", "root", "secret");
        creator.create();
        Connection connection = DriverManager.getConnection("jdbc:h2:tcp://localhost:3456/myDB", "root", "secret");
        connection.close();
    }
