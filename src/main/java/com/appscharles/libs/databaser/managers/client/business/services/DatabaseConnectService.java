...
import com.appscharles.libs.databaser.initializers.ISessionFactoryInitializer;
import com.appscharles.libs.databaser.initializers.SessionFactoryInitializer;
...
import com.appscharles.libs.databaser.validators.*;
...
public class DatabaseConnectService {
    ...
    public Boolean connect() throws DatabaserException {
        ...
        if (ConnectionValidator.isValid(databaseURL, user, password, this.controller.clientManagerConfiguration.getResourceMigrationPath()) == false) {
            AlertFactory.create(Alert.AlertType.ERROR, this.controller.resourceBundle.getString("view.dialog.exception.title"), this.controller.resourceBundle
                    .getString("view.dialog.exception.failed_connection_valid")).setIconStageResource("/com/appscharles/libs/databaser/managers/client/ClientManagerIcon.png").build().showAndWait();
            return false;
        }

        ISessionFactoryInitializer initializer = new SessionFactoryInitializer(databaseURL, user, password, this.controller.clientManagerConfiguration.getSessionFactoryConsumer());
